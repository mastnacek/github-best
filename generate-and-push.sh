...
if command -v node >/dev/null 2>&1 && [ -f "$SCRIPT_DIR/generate-and-push.ts" ]; then
    echo "Executing TypeScript generator via Node.js..."
    node "$SCRIPT_DIR/generate-and-push.ts"
elif command -v python3 >/dev/null 2>&1 && [ -f "$SCRIPT_DIR/generate_and_push.py" ]; then
    ...
    echo "Error: Node.js or Python 3 is required to run the data collection." >&2
    exit 1
fi
